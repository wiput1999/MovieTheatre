      ...
      <div class="container">
        <div class="wrapper">
          ...
          <div class="movie-detail">
            <h1>{{movie.title}}</h1>
            <div class="header">
              <div v-for="d in dateList" :key="d.date" v-bind:class="{active: today.format('MMM DD') === d.date}">
            ...
            <div class="seat-wrapper">
              ...
              <div class=row>
                <div v-for="c in 10" :key="`${r}${c}`" class="">
              ...
              <div class="row" v-for="r in letters.substring(0,row).split('').reverse()" :key="r">
                ...
                <div class="box" v-for="c in column" v-bind:class="getClasses(seatSelected, seatBooked, `${r}${c}`)"
                  @click="onSeatClick(`${r}${c}`)" :key="`${r}${c}`"></div>
              </div>
              <div style="background-color:white; height:5px; margin-top:40px">
              </div>
              <div style="margin-top:10px; ">
                <center>THEATER SCREEN</center>
              </div>
  ...
  export default {
    ...
    computed: {
      dateList() {
        ...
        for (let i = -1; i < 2; i++) {
          dateList.push({
            date: this.today.add(i, 'day').format('MMM DD'),
            day: this.today.add(i, 'day').format('ddd').toUpperCase()
          })
        }
        return dateList
      },
      ...
      seatBooked() {
        return this.$store.state.seatBooked
      },
      seatSelected() {
        return this.$store.state.seatSelected
      }
